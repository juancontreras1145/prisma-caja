const STORAGE_KEY = "cajaMinimalDataV2";
    ...
    function saveSale() {
      ...
      currentAccumulatedReceipt = null;
    ...
    function syncSalePaymentState(sale) {
      if (!sale || sale.type !== "venta") return sale;
      ...
      if (total > 0 && paid >= total) {
        ...
        if (!sale.lastPaymentMethod) sale.lastPaymentMethod = sale.method || "Pagado";
      } else if (paid > 0) {
        sale.status = "abonado";
        sale.paymentStatus = "abonado";
      } else {
        sale.status = "fiado";
    ...
    function syncAllSalePaymentStates() {
      data.movements.forEach(m => {
        if (m.type === "venta") syncSalePaymentState(m);
      });
    }
    ...
    function updateHistoryTabs() {
      const labels = {
        boletas: "Boletas",
        abonos: "Abonos",
        pendientes: "Pendientes Acumulados"
      };

      document.getElementById("historyTabBoletas")?.classList.toggle("active", activeHistoryTab === "boletas");
      document.getElementById("historyTabAbonos")?.classList.toggle("active", activeHistoryTab === "abonos");
      document.getElementById("historyTabPendientes")?.classList.toggle("active", activeHistoryTab === "pendientes");

      const label = document.getElementById("historyLabel");
      if (label) label.textContent = labels[activeHistoryTab] || "Boletas";
    }
    ...
    function renderHistory() {
      ...
      if (activeHistoryTab === "pendientes") {
        renderPendingAccumulated(target);
        return;
      }

      const movements = data.movements.filter(m => activeHistoryTab === "abonos" ? m.type === "abono" : m.type === "venta");
      const emptyText = activeHistoryTab === "abonos" ? "Todavía no hay abonos." : "Todavía no hay boletas.";
    ...
    function pendingClientKeyFromSale(sale) {
      if (sale.clientId) return "id:" + sale.clientId;
      return "name:" + normalize(sale.clientName || "sin cliente");
    }

    function getClientForSale(sale) {
    ...
    function getPendingAccumulatedClients() {
      ...
      getSales().forEach(sale => {
        ...
        const key = pendingClientKeyFromSale(sale);
        const client = getClientForSale(sale);
        const current = map.get(key) || {
          key,
          clientId: client?.id || sale.clientId || "",
          clientName: client?.name || sale.clientName || "Cliente",
          clientPhone: sanitizePhone(client?.phone || sale.clientPhone || ""),
          sales: [],
          pending: 0,
          total: 0,
          paid: 0
        ...
        current.paid += Number(sale.amountPaid || 0);
        if (!current.clientPhone) current.clientPhone = sanitizePhone(client?.phone || sale.clientPhone || "");
        map.set(key, current);
      });

      return [...map.values()].sort((a, b) => b.pending - a.pending || a.clientName.localeCompare(b.clientName));
    }

    function renderPendingAccumulated(target) {
      const clients = getPendingAccumulatedClients();

      if (!clients.length) {
        target.innerHTML = '<div class="empty">No hay clientes con boletas pendientes.</div>';
        return;
      }

      target.innerHTML = clients.map(client => {
        const receiptNumbers = client.sales.map(sale => formatReceiptNumber(sale.boletaNumber)).join(", ");
        const dates = [...new Set(client.sales.map(sale => formatShortDate(sale.createdAt)).filter(Boolean))].join(", ");
        return `
          <div class="card">
            <div class="card-left">
              <strong>${escapeHtml(client.clientName)}</strong>
              <div class="pending-summary">
                <small>${client.sales.length} boleta(s) pendiente(s) · ${escapeHtml(dates || "Sin fecha")}</small>
                <small>Boletas ${escapeHtml(receiptNumbers)}</small>
                <small>Total acumulado: ${formatMoney(client.pending)}</small>
              </div>
              <span class="badge debt">Pendiente</span>
            </div>
            <div class="card-actions">
              <button class="share-btn" onclick="showAccumulatedReceipt('${escapeAttr(encodeURIComponent(client.key))}')">Boleta acumulada</button>
            </div>
          </div>
        `;
      }).join("");
    ...
    function buildAccumulatedReceiptFromKey(encodedKey) {
      const key = decodeURIComponent(String(encodedKey || ""));
      const client = getPendingAccumulatedClients().find(item => item.key === key);
      if (!client) return null;

      const itemMap = new Map();
      client.sales.forEach(sale => {
        const dateLabel = formatShortDate(sale.createdAt);
        (sale.items || []).forEach(item => {
          const name = String(item.name || "Producto");
          const itemKey = `${normalize(name)}|${dateLabel}|${Number(item.price || 0)}`;
      ...
      return {
        ...
        sales: client.sales,
        receiptNumbers: client.sales.map(sale => formatReceiptNumber(sale.boletaNumber)),
        items: [...itemMap.values()],
        total: client.total,
        amountPaid: client.paid,
        remaining: client.pending,
        createdAt: new Date().toISOString()
      };
    }

    function showAccumulatedReceipt(encodedKey) {
      const receipt = buildAccumulatedReceiptFromKey(encodedKey);
      if (!receipt || !receipt.sales.length) {
        toast("No hay boletas pendientes para este cliente");
        return;
      }

      currentReceiptSaleId = null;
      currentAccumulatedReceipt = receipt;

      setReceiptModalTitle("Boleta acumulada");
      const target = document.getElementById("receiptPreview");
      target.innerHTML = "";
      target.appendChild(createAccumulatedReceiptNode(receipt));
      removeReceiptDeleteButton();
    ...
    function setReceiptModalTitle(text) {
      const title = document.getElementById("receiptModalTitle");
      if (title) title.textContent = text;
    }

    function removeReceiptDeleteButton() {
      const modal = document.getElementById("receiptModal");
      const zone = modal?.querySelector(".safe-delete-zone");
      if (zone) zone.remove();
    }
    ...
    function currentReceipt() {
      return currentAccumulatedReceipt || currentSale();
    }

    function createAccumulatedReceiptNode(receipt) {
      const card = document.createElement("div");
      card.className = "receipt-card";

      const itemsHtml = (receipt.items || []).map(item => `
        <div class="receipt-item accumulated">
          ...
          <div class="receipt-item-date">${escapeHtml(item.dateLabel || "")}</div>
        </div>
      `).join("");

      card.innerHTML = `
        <div class="receipt-head">
          <div class="receipt-title">CLIENTE: ${escapeHtml((receipt.clientName || "CLIENTE").toUpperCase())}</div>
          <div class="receipt-sub">Pendientes acumulados · ${escapeHtml(formatDateTime(receipt.createdAt))}</div>
        </div>

        <div class="receipt-meta">
          <div class="receipt-row"><span class="receipt-left">Boletas</span><span class="receipt-right">${escapeHtml((receipt.receiptNumbers || []).join(", "))}</span></div>
          <div class="receipt-row"><span class="receipt-left">Pago</span><span class="receipt-right">Pendiente</span></div>
          <div class="receipt-row"><span class="receipt-left">Pendiente total</span><span class="receipt-right">${escapeHtml(formatMoney(receipt.remaining))}</span></div>
        </div>
        <div class="receipt-section">
          <div class="receipt-table-head accumulated"><div>Producto</div><div style="text-align:right">Cant.</div><div style="text-align:right">Total</div><div style="text-align:right">Fecha</div></div>
          ${itemsHtml}
        </div>

        <div class="receipt-summary">
          <div class="receipt-line"><span>Total</span><strong>${formatMoney(receipt.total)}</strong></div>
          <div class="receipt-line"><span>Abonado</span><span>${formatMoney(receipt.amountPaid || 0)}</span></div>
          <div class="receipt-line"><span>Saldo acumulado</span><span>${formatMoney(receipt.remaining)}</span></div>
        </div>

        <div class="receipt-footer">
          <div>Boletas acumuladas</div>
    ...
    async function compartirImagenWhatsApp() {
      ...
      try {
        const isAccumulated = receipt.type === "acumulado";
        const blob = isAccumulated ? await buildAccumulatedReceiptImageCanvas(receipt) : await buildReceiptImageCanvas(receipt);
        const filename = isAccumulated
          ? `pendientes-${safeFileName(receipt.clientName || "cliente")}.png`
          : `boleta-${formatReceiptNumber(receipt.boletaNumber)}-${safeFileName(receipt.clientName || "cliente")}.png`;
        const phone = isAccumulated ? sanitizePhone(receipt.clientPhone || "") : phoneForSale(receipt);
    ...
    async function buildAccumulatedReceiptImageCanvas(receipt) {
      const scale = window.AndroidBridge ? 1.35 : 2;
      const width = 760;
      const margin = 42;
      const items = receipt.items || [];
      const height = 710 + Math.max(1, items.length) * 68;
      ...
      ctx.fillStyle = black;
      ctx.font = "900 36px Arial";
      fitCenteredCanvasText(ctx, "CLIENTE: " + (receipt.clientName || "CLIENTE").toUpperCase(), width / 2, y, width - margin * 2);
      y += 36;

      ctx.fillStyle = muted;
      ctx.font = "24px Arial";
      ctx.fillText("PENDIENTES ACUMULADOS", width / 2, y);
      y += 40;

      dashedLine(ctx, margin, y, width - margin, y, dash);
      y += 34;

      y = canvasPair(ctx, "Boletas", (receipt.receiptNumbers || []).join(", "), margin, width - margin, y);
      y = canvasPair(ctx, "Pago", "Pendiente", margin, width - margin, y);
      y = canvasPair(ctx, "Pendiente total", formatMoney(receipt.remaining), margin, width - margin, y);
      ...
      ctx.fillText("PRODUCTO", margin, y);
      ctx.textAlign = "right";
      ctx.fillText("CANT.", width - margin - 210, y);
      ctx.fillText("TOTAL", width - margin - 72, y);
      ctx.fillText("FECHA", width - margin, y);
      y += 22;
      ...
      items.forEach(item => {
        ...
        fillCanvasTextEllipsis(ctx, item.name, margin, y, width - margin * 2 - 330);

        ctx.textAlign = "right";
        ctx.font = "25px Arial";
        ctx.fillText(String(item.qty), width - margin - 220, y);

        ctx.font = "900 25px Arial";
        ctx.fillText(formatMoney(item.total), width - margin - 72, y);

        ctx.font = "25px Arial";
        ctx.fillText(item.dateLabel || "", width - margin, y);
        y += 60;
      });
      ...
      ctx.textAlign = "right";
      ctx.font = "900 44px Arial";
      ctx.fillText(formatMoney(receipt.total), width - margin, y);
      y += 40;
      ...
      ctx.fillText("ABONADO", margin, y);
      ctx.textAlign = "right";
      ctx.fillText(formatMoney(receipt.amountPaid || 0), width - margin, y);
      y += 34;

      ctx.textAlign = "left";
      ctx.fillText("SALDO ACUMULADO", margin, y);
      ctx.textAlign = "right";
      ctx.fillText(formatMoney(receipt.remaining), width - margin, y);
      y += 56;
      ...
      ctx.fillStyle = muted;
      ctx.font = "24px Arial";
      ctx.fillText("Boletas acumuladas", width / 2, y);
      y += 30;
      ctx.fillText("Gracias por su compra", width / 2, y);
    ...
    function fillCanvasTextEllipsis(ctx, text, x, y, maxWidth) {
