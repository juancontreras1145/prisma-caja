...
public class MainActivity extends Activity {
    ...
    private void deliverUpdateResult(JSONObject result) {
        ...
        webView.post(() -> webView.evaluateJavascript(
                "window.handleNativeUpdateResult && window.handleNativeUpdateResult(" + payload + ");",
                null
        ));
    }
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (status == 404) {
                    ...
                    result.put("ok", true);
                    result.put("tag_name", release.optString("tag_name", ""));
                    result.put("name", release.optString("name", ""));
                    result.put("html_url", release.optString("html_url", ""));
